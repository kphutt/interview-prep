...
- Model-agnostic API calls (backlog item, not yet implemented)
...
## Phase 2 — Template unification + syllabus adaptation [NOT STARTED]
...
## Phase 3 — Profile directory + config [NOT STARTED]
...
profiles/
  security-infra/              # migrated from current outputs/ (decision #13)
    ...
    inputs/                    # JD, notes, context docs
      ...
      notes.md
      context/
    adapted/                   # generated by `adapt` command (decision #15)
...
Profile: my-prep (Senior SWE @ Meta)
  ...
  Pipeline:
    ...
    [ ] Domain adapted           profiles/my-prep/adapted/ (0/4 files)
  ...
  Next: python prep.py adapt --profile my-prep
```
...
## Phase 4 — Domain-agnostic prompts [NOT STARTED]
...
#### 4.1: Define adapted section file format

Create `profiles/{name}/adapted/` directory (decision #15 — not `domain/`, to signal these are generated intermediates, not user inputs) with files that contain injectable content.
...
```markdown
<!-- Generated by: python prep.py adapt --profile {name} -->
<!-- To regenerate: python prep.py adapt --profile {name} --force -->
```
...
**Injection function:** Create `inject_adapted(text, profile_name)` that reads adapted files and substitutes markers. Separate from `render_template()` — render handles 5 simple env vars; injection handles multi-line file content.

**Fallback:** When no adapted files exist (legacy mode or profile without `adapt` run), use hardcoded S&I content as default. This keeps the tool working during the transition.

**Tests:**
- `test_inject_reads_adapted_files` — content from adapted/ appears in rendered prompt
- `test_inject_without_adapted_uses_defaults` — S&I content used when adapted files missing
- `test_no_domain_leakage` — with custom adapted files, no S&I terms in output
- `test_braces_in_adapted_content` — injected `{json: "example"}` doesn't break substitution

#### 4.3: Extract current S&I content into adapted files

Move current hardcoded content from prompts into `profiles/security-infra/adapted/` files. The prompts become templates; S&I becomes one instance.

**Tests:**
- `test_si_adapted_files_created` — all 4 files exist for security-infra
...
5. If no: keep frames but make them domain-injectable. Add `frames.md` to adapted files. Meta-prompt (Phase 5) generates per-episode frames.
...
- `test_adapted_frames_injected` — frames from adapted/ used in packaging
...
- **Phase 4 state:** `render --profile` also injects adapted files (if they exist)
- **If adapted files don't exist:** render substitutes vars only, prints warning: `Note: adapted/ files not found. Run 'python prep.py adapt --profile my-prep' for full rendering.`

**Tests:**
- `test_render_profile_substitutes_vars` — PREP_* replaced from profile
- `test_render_profile_injects_adapted` — adapted content injected
- `test_render_profile_warns_no_adapted` — warning when adapted files missing
...
|--------|-------------------|--------|
| `syllabus.md` | Training data (~120 lines), coverage framework, RRK rules, frontier rules | `profiles/{name}/adapted/seeds.md`, `profiles/{name}/adapted/coverage.md` |
| `content.md` | Domain lens, RRK lens, audience, Nitty Gritty subsection names, requirements, stakeholders, portability examples | `profiles/{name}/adapted/lenses.md` |
| `distill.md` | Same as content.md (shares injection points) | Same source |
| `gem.md` | Persona names/descriptions, Bookshelf, example questions, Light Coding framing | `profiles/{name}/adapted/gem-sections.md` |
| `notebooklm.md` | Host 2 translation lens (minor) | Inline in profile.md |
| `notebooklm-frames.md` | Eliminated if decision #1 validates; otherwise domain-injectable | N/A or `profiles/{name}/adapted/frames.md` |

---

## Phase 5 — Intake + meta-prompt [NOT STARTED]
...
**Resolved open questions:**
- Q13 (one meta-prompt or several) — three focused calls: seeds+coverage, lenses, gem-sections (decision implicit in Phase 5.2)
...
**Output:** The files in `profiles/{name}/adapted/`:
...
#### 5.2: Split meta-prompt into focused API calls

Instead of one monolithic generation, use three focused API calls:

| Call | Input | Output | Why separate |
|------|-------|--------|-------------|
| 1: Seeds + Coverage | profile.md, JD, context docs | `seeds.md`, `coverage.md` | Most critical output — needs full context window. Seeds determine syllabus quality. Coverage and seeds are tightly coupled. |
| 2: Lenses + Structure | profile.md | `lenses.md` | Domain/RRK lenses, Nitty Gritty subsections, stakeholders. Smaller output, focused task. |
| 3: Gem sections | profile.md, seeds.md (from call 1) | `gem-sections.md` | Bookshelf, example questions, persona descriptions. Benefits from seeing the seeds. |

Write three prompt files:
- `prompts/meta-seeds.md` — generates seeds.md + coverage.md
- `prompts/meta-lenses.md` — generates lenses.md
- `prompts/meta-gem.md` — generates gem-sections.md

**Tests:**
- `test_meta_seeds_prompt_renders`
- `test_meta_lenses_prompt_renders`
- `test_meta_gem_prompt_renders`
- Manual testing: feed S&I profile, verify output quality matches current domain content

#### 5.3: `cmd_adapt(name)` command (decision #14)

```bash
python prep.py adapt --profile my-prep
```

Reads `profiles/{name}/profile.md` + inputs, runs three meta-prompt API calls (5.2), writes adapted files to `profiles/{name}/adapted/`.

**Cost estimate:** ~$5-10 (3 API calls). Printed before proceeding (reuses Phase 3.6 mechanism).

**Tests:**
- `test_adapt_creates_all_adapted_files` — all 4 files created
- `test_adapt_requires_profile` — error without --profile
- `test_adapt_reads_context_docs` — context/ docs included in API input
- `test_adapt_force_regenerates` — `--force` overwrites existing adapted files
...
# → Prints: "Next: python prep.py adapt --profile my-prep"

# Step 4: Generate domain-specific adapted sections (~$5-10, 3 API calls)
python prep.py adapt --profile my-prep
# → Creates adapted/seeds.md, coverage.md, lenses.md, gem-sections.md
...
Syllabus generated. Review before running content generation (~$25):
  ...
  [ ] Topics cover JD requirements (cross-reference with adapted/coverage.md)
...
- **3 before 4:** Profile directory structure and `set_profile()` mechanism must exist before adapted files can live in profiles
- **4 before 5:** Injection markers and adapted file format must be defined before the meta-prompt can target them
