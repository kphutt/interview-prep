...
profiles/
  security-infra/              # migrated from current outputs/ (decision #13)
    ...
    inputs/                    # JD, notes, context docs
      ...
      notes.md
      context/
    adapted/                   # generated by `setup` command (decision #15)
...
**Fallback:** When no adapted files exist (legacy mode or profile without `setup` run), use hardcoded S&I content as default. This keeps the tool working during the transition.
...
- **If adapted files don't exist:** render substitutes vars only, prints warning: `Note: adapted/ files not found. Run 'python prep.py setup --profile my-prep' for full rendering.`
...
# Step 4: Generate domain-specific adapted sections (~$5-10, 1 API call)
