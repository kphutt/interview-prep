...
14. **`generate-prompts` command renamed to `setup`** — the command generates domain injection fragments (seeds.md, coverage.md, lenses.md, gem-sections.md), not prompts. Implemented as a single API call (not the three meta-prompt calls from Phase 5.2) using `prompts/setup.md`.

15. **`domain/` directory renamed to `adapted/`** — `profiles/{name}/adapted/` signals "machine-generated intermediates," not "user-created domain inputs." Generated by the `setup` command.
...
19. **`adapted/` renamed to `domain/`** — "adapted" was confusing (sounds like a past-tense verb, not a noun). The directory holds domain-specific content chunks (seeds, coverage framework, lenses, gem sections) that get injected into prompt templates. `domain/` is simpler and reads naturally under `profiles/{name}/domain/`. Supersedes decision #15.
